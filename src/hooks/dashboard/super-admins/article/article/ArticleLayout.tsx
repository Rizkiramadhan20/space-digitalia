...
import { Article, Category } from '@/hooks/dashboard/super-admins/article/article/lib/schema'

import ArticleModal from '@/hooks/dashboard/super-admins/article/article/content/ArticleModal'

import Image from 'next/image'

import ArticleSkeleton from '@/hooks/dashboard/super-admins/article/article/ArticleSkelaton'
...
export default function ArticleLayout() {
    ...
    const handleEdit = (article: Article) => {
        // Check if user is the author or has super-admin role
        if (article.author.uid !== user?.uid && !hasRole(['super-admins'])) {
                        ...
                        <div className="p-5">
                            ...
                            <div className="flex justify-end gap-2 pt-3 border-t">
                                ...
                                <button
                                    ...
                                    className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${article.author.uid === user?.uid || hasRole(['super-admins'])
                                        ? 'text-white bg-indigo-600 hover:bg-indigo-700'
                                        : 'text-gray-400 bg-gray-100 cursor-not-allowed'
                                        }`}
                                    disabled={article.author.uid !== user?.uid && !hasRole(['super-admins'])}
                                    title={article.author.uid === user?.uid || hasRole(['super-admins'])
                                        ? "Edit article"
