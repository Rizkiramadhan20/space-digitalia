...
export default function CheckoutContent() {
    ...
    const handlePayment = async () => {
        try {
            if (!user?.uid || !project || !licenseType) {
            ...
            const selectedLicense = project.licenseDetails.find(
                license => license.title === licenseType
            )

            if (!selectedLicense) {
                throw new Error('Selected license not found')
            }

            // Handle paid products with Midtrans
            const response = await fetch('/api/payment', {
                ...
                body: JSON.stringify({
                    ...
                    deliveryAddress: deliveryMethod === 'delivery' ? defaultAddress : undefined,
                }),
            })
            ...
            // Initialize Midtrans Snap for paid products
            window.snap.pay(data.token, {
                onSuccess: async (result) => {
                    await handlePaymentStatus(data.orderId, 'success', result)
                    router.push(`/payment/status/${data.orderId}`)
                },
                onPending: async (result) => {
                    await handlePaymentStatus(data.orderId, 'pending', result)
                    router.push(`/payment/status/${data.orderId}`)
                },
                onError: async (result) => {
                    await handlePaymentStatus(data.orderId, 'failure', result)
                    router.push(`/payment/status/${data.orderId}`)
                },
                onClose: async () => {
                    setIsProcessing(false)
                    // Ensure we wait for any pending status updates before redirecting
                    await router.push(`/payment/status/${data.orderId}`)
                },
            })
    ...
    const handlePaymentStatus = async (orderId: string, status: string, result: MidtransResult) => {
        try {
