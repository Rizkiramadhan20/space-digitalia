    ...
    useEffect(() => {
        const fetchRatings = async () => {
            try {
                ...
                // Check if there's ratingCount in the project document
                const ratingCount = projectData.ratingCount || 0;

                if (ratingCount === 0) {
                    console.log('No ratings yet for this project');
                    setRatings([]);
                    return;
                }

                // Try to fetch ratings from the subcollection
                try {
                    console.log('Fetching ratings from subcollection...');
                    const ratingsRef = collection(db, 'projects', slug, 'ratings');
                    const ratingsSnapshot = await getDocs(ratingsRef);

                    if (!ratingsSnapshot.empty) {
                        console.log(`Found ${ratingsSnapshot.size} ratings in subcollection`);
                        ...
                        console.log('No ratings in subcollection');
                        setRatings([]);
                    }
                } catch (error) {
                                ...
                                <div className="mt-8">
                                    ...
                                    {ratings.length > 0 ? (
                                        ...
                                        <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 text-center">
                                            <p className="text-gray-500">No ratings yet for this project. Be the first to rate!</p>
                                        </div>
                                    )}
                                </div>
